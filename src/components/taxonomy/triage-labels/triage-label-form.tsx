...
import { LABEL_GROUPS, type TriageLabel } from "@/lib/models/triage-label";
...
const LABEL_GROUP_LABEL: Record<(typeof LABEL_GROUPS)[number], string> = {
  safety: "Safety",
  urgency: "Urgency",
  context_indicator: "Context indicator (never a confirmed finding)",
  bias_indicator: "Bias indicator (never a confirmed finding)",
  support_need: "Support need",
  accessibility_need: "Accessibility need",
  other: "Other",
};

export function TriageLabelForm({ mode, initialRecord }: { mode: "create" | "edit"; initialRecord?: TriageLabel | null }) {
  const { repository } = useAdminRepository();
  const router = useRouter();
  const existingAll = useCrudList((repo) => repo.triageLabels) ?? [];

  const [record, setRecord] = useState<TriageLabel | null>(initialRecord ?? null);
  ...
  const [labelGroup, setLabelGroup] = useState<TriageLabel["labelGroup"]>(initialRecord?.labelGroup ?? "other");
  ...
  function handleCancel() {
    ...
    else router.push("/taxonomy/triage-labels");
  }
  ...
  async function handleSave(target: ContentStatus) {
    ...
    if (mode === "create") {
      ...
      if (isDuplicateMachineKey(normalizedKey, existingAll)) {
        setKeyError("This stable key is already used by another triage label.");
        return;
      }
    }
    setKeyError(null);
    ...
    try {
      ...
      if (!record) {
        ...
        const draft: TriageLabel = {
          ...createBaseFields({ status: target }),
          machineKey: normalizedKey,
          urgencyLevel: "moderate",
          ...fields,
        };
        saved = await repository.triageLabels.create(draft);
      } else {
        saved = await repository.triageLabels.updateWithVersionCheck(record.id, fields, record.version, LOCAL_ADMIN_ACTOR);
      }

      if (saved.status !== target) {
        saved = await repository.triageLabels.transitionStatus(saved.id, target, LOCAL_ADMIN_ACTOR);
      }

      setRecord(saved);
      setIsDirty(false);
      router.push(`/taxonomy/triage-labels/${saved.id}` as Route);
    } catch (err) {
  ...
  const previewRecord: TriageLabel = {
    ...(record ?? { ...createBaseFields({ status: "draft" }), machineKey, urgencyLevel: "moderate" }),
    name: name.trim(),
    machineKey,
    description: description.trim() || undefined,
    labelGroup,
  ...
  const blockers = getTriageLabelBlockers(
    previewRecord,
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-5 rounded-xl border border-border bg-card p-5 sm:p-6">
        <div className="space-y-1.5">
          <Label htmlFor="name">Name shown to administrators</Label>
          <Input
            id="name"
            value={name}
            onChange={(event) => {
        ...
        {labelGroup === "bias_indicator" || labelGroup === "context_indicator" ? (
      ...
      <Dialog open={cancelConfirmOpen} onOpenChange={setCancelConfirmOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => router.push("/taxonomy/triage-labels")}>
              Discard changes
            </Button>
          </DialogFooter>
        </DialogContent>
